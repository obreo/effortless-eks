...
resource "aws_subnet" "private" {
  ...
  vpc_id                                          = aws_vpc.vpc.id
  dynamic "cidr_block" {
    for_each = var.vpc_settings.enable_aws_ipv6_cidr_block.ipv6_native || length(var.vpc_settings.private_subnet_cidr_blocks) == 0 ? [] : [1]
    content {
      cidr_block = var.vpc_settings.private_subnet_cidr_blocks[count.index]
    }
  }
